...
로컬 개발(`docker-compose.yml`, 호스트 포트 3309/8081)과 별개로, 홈서버(FIREBAT R3)에는 `docker-compose.prod.yml`로 MySQL + Tomcat(앱) 두 컨테이너를 함께 올린다. 컨테이너 **내부** 포트는 표준값(MySQL 3306, Tomcat 8080) 그대로 쓰고, **호스트** 매핑 포트만 서버에 이미 떠 있는 다른 프로젝트(hsmarket, routfit, concert-ticketing 등)와 겹치지 않게 새로 골랐다.

| 서비스 | 컨테이너 내부 포트 | 호스트 매핑 포트 | 비고 |
|---|---|---|---|
| MySQL | 3306 | **3308** | 서버에서 직접 접속/점검용. 3306/3307은 이미 다른 프로젝트가 사용 중 |
| Tomcat(앱) | 8080 | **8082** | 호스트에서 직접 점검할 때만 사용 — 8080/8081은 hsmarket이 사용 중 |

### Nginx Proxy Manager 연결

이 서버의 기존 4개 사이트(hsmarket/routfit/routfit-api/concert-ticketing)를 확인해보니, 전부 **호스트 포트가 아니라 컨테이너명 + 내부 표준 포트**로 NPM이 직접 프록시하고 있었다(NPM 컨테이너가 각 프로젝트의 도커 네트워크에 join되어 있음). hanbit-erp도 이 방식을 그대로 따른다.

```bash
# 1. 서버에서 .env 준비 (로컬 dev용 .env와 값을 공유하지 않는다 — 서버 전용 값으로 새로 채움)
cp .env.example .env

# 2. MySQL + 앱 기동
docker compose -f docker-compose.prod.yml up -d --build

# 3. NPM 컨테이너를 hanbit-erp 전용 네트워크에 연결 (최초 1회만)
docker network connect hanbit-erp-net npm-npm-1
```

NPM 관리 화면(Proxy Hosts → Add Proxy Host)에서:

| 항목 | 값 |
|---|---|
| Domain Names | `hanbit-erp.duckdns.org` |
| Forward Hostname / IP | `hanbit-erp-app` (호스트 포트 8082가 아니라 컨테이너명) |
| Forward Port | `8080` (컨테이너 내부 표준 포트) |
| SSL | Let's Encrypt로 인증서 발급 (기존 4개 사이트와 동일) |

이렇게 하면 호스트 포트(3308/8082)는 서버에서 직접 점검할 때만 쓰이고, 실제 공개 트래픽은 도커 네트워크 내부로만 흐른다 — 다른 4개 프로젝트와 동일한 보안 구성이다.
