...
| FR-4-4 | 구현됨 | `ExpenseServiceImpl.requestExpense()` — 80~100%는 경고 표시, 100% 초과 시 상신 차단(500만원 이상은 예외 사유 입력 시 허용) | 2026-09-13: 요구사항 정의서를 실제 구현(상신 게이트 방식)에 맞춰 갱신함. 결재 단계 추가 자체는 FR-2-4(금액 구간)가 전담 |
...
### [해결: 2026-09-13] FR-2-4 / FR-4-4 — "예산 80% 초과 시 대표이사 결재 추가"가 구현되지 않음
- **원인**: 지출결의서 결재 단계 추가 조건이 요구사항 원문("부서 예산 80% 초과")과 달리 실제로는 지출 금액 자체(500만원 이상)로 결정되고 있었음(ADR-011로 이미 의도적으로 변경됐으나 `erp_requirements.md`가 갱신되지 않음)
- **처리**: 코드가 아니라 요구사항 정의서 쪽이 실제 구현에 맞게 갱신됨 — 금액 구간 방식이 예산 소진율 방식보다 더 단순하고, 상신 시점에 고정돼야 하는 결재라인을 부서·시점마다 변하는 예산 소진율과 결합하지 않아도 된다는 점에서 합리적인 설계로 판단. `docs/erp_requirements.md`의 결재 유형별 승인 단계 표, FR-2-4, FR-4-4에 변경 이력과 근거(ADR-011)를 각주로 남김. 상세 경위는 `docs/troubleshooting.md` 2026-09-13 항목 참고

### [해결: 2026-09-13] 구매요청서 결재라인이 "물류본부는 항상 3단계"에서 "금액에 따라 1~3단계"로 축소됨
...
### 1. [Medium] FR-5-1 — 재고 입고/출고 이력 조회 기능이 전혀 없음
...
### 2. [Medium] NFR-2-1 — 결재 상태 변경 알림(실시간/준실시간)이 전혀 없음
