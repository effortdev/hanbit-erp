...
- **별개로 발견한 것 (이번 항목과 무관, 해결하지 않음)**: 검증 중 우연히 `maximumSessions(1)` 동시 로그인 제어(ADR-005)가 실제로는 동작하지 않는 것을 발견했다 — 같은 계정으로 두 번째 로그인을 해도 첫 번째 세션이 만료되지 않고 계속 유효했다. Jetty와 Tomcat 양쪽에서 동일하게 재현되어 컨테이너 차이가 아니라 앱 설정 자체의 기존 결함으로 보인다. 이번 검증(WAR+Tomcat 조합 확인)의 범위를 벗어나 여기서는 고치지 않았고, 별도로 원인 조사가 필요하다.
