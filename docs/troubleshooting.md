...
## 2026-09-14 — WAR + Tomcat 9 배포 검증 중 `web.xml`의 Servlet 버전이 실제 요구사항보다 낮게 남아있었음

- **증상(엄밀히는 장애 아님)**: `mvn clean package`로 만든 WAR를 `tomcat:9-jdk17` 컨테이너(도커 네트워크로 기존 `docker-compose`의 MySQL과 연결)에 배포해 로그인→조직도 조회→휴가신청 기안까지 curl로 검증했을 때는 아무 문제 없이 전부 통과했다. 다만 점검 과정에서 `web.xml`이 여전히 `version="3.0"`(`web-app_3_0.xsd`)으로 선언돼 있는 걸 발견했다.
- **원인**: 이 값은 애초에 `tomcat7-maven-plugin`(내장 Tomcat 7.0.47, Servlet 3.0)에 맞춰 다운그레이드했던 것인데(아래 tomcat7 관련 항목 참고), 그 이후 로컬 실행 도구를 Jetty 9.4(Servlet 3.1)로 바꾸고 이번에 실제 배포 대상을 Tomcat 9(Servlet 4.0)로 검증하는 동안에도 갱신되지 않고 그대로 남아있었다. 정작 애플리케이션은 Spring Security의 `HttpSessionEventPublisher`가 구현하는 `HttpSessionIdListener`(Servlet 3.1+ 인터페이스)에 의존한다 — 즉 `web.xml`이 선언한 스펙 버전과 실제 요구 버전이 어긋나 있었다. Tomcat/Jetty 모두 컨테이너가 지원하는 실제 API 가용성으로 동작하고 `web.xml`의 선언 버전 자체를 강하게 강제하지는 않아 지금까지 겉으로 드러나는 오류는 없었다.
- **해결**: `web.xml`을 실제 요구 버전 이상이자 Tomcat 9가 지원하는 최신 스펙인 `version="4.0"`(`web-app_4_0.xsd`)으로 올렸다. 변경 후 WAR를 다시 빌드해 Tomcat 9 컨테이너에 재배포하고 동일한 curl 검증(로그인/조직도/기안)을 재실행해 회귀 없음을 확인했다.
- **별개로 발견한 것 (이번 항목과 무관)**: 검증 중 우연히 `maximumSessions(1)` 동시 로그인 제어(ADR-005)가 실제로는 동작하지 않는 것을 발견했다 — 같은 계정으로 두 번째 로그인을 해도 첫 번째 세션이 만료되지 않고 계속 유효했다. Jetty와 Tomcat 양쪽에서 동일하게 재현되어 컨테이너 차이가 아니라 앱 설정 자체의 기존 결함이었다. 이번 검증(WAR+Tomcat 조합 확인)의 범위를 벗어나 당시에는 고치지 않았으나, 원인 조사 후 별도로 수정했다 — 원인과 수정 내용은 `docs/adr/ADR-005-authentication.md`의 "후속 기록" 참고.
